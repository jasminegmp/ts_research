function gui_ds = new_sum_fastMPdist(merge_type,f_name, segment_length)%% Modifiable constants
    ...
    gui_ds = insert_gd(gui_ds,(1:length(ts_1)), ts_1, gd_count, 1);
    gui_ds = insert_gd(gui_ds,NaN, NaN, gd_count, 3);
    gui_ds = insert_gd(gui_ds,NaN, NaN, gd_count, 5);
    gd_count = gd_count + 1;
    ...
    %         plot(loc_2:loc_3, m_seg_1, 'Color', [0.1 0.1 0.1], 'LineWidth', 1.2)
            gui_ds = insert_gd(gui_ds,(1:length(ts_1)), ts_1, gd_count, 1);
            gui_ds = insert_gd(gui_ds,(loc_0:loc_1), m_seg_0, gd_count, 3);
            gui_ds = insert_gd(gui_ds,(loc_2:loc_3), m_seg_1, gd_count, 5);
            gd_count = gd_count + 1;
...
%         
            gui_ds = insert_gd(gui_ds,(1:length(ts_1)), ts_1, gd_count, 1);
            if merged_data{merge_count,7}{1,1} == merged_data{merge_count,1}{1,1}
                gui_ds = insert_gd(gui_ds,(merged_data{merge_count,1}{1,1}:merged_data{merge_count,2}{1,1}), merged_data{merge_count,3}{1,1}, gd_count, 3);
                gui_ds = insert_gd(gui_ds,(merged_data{merge_count,4}{1,1}:merged_data{merge_count,5}{1,1}), merged_data{merge_count,6}{1,1}, gd_count, 5);
            else
                gui_ds = insert_gd(gui_ds,(merged_data{merge_count,4}{1,1}:merged_data{merge_count,5}{1,1}), merged_data{merge_count,6}{1,1}, gd_count, 3);
                gui_ds = insert_gd(gui_ds,(merged_data{merge_count,1}{1,1}:merged_data{merge_count,2}{1,1}), merged_data{merge_count,3}{1,1}, gd_count, 5);
            end
...
function [gui_ds] = insert_gd(gui_ds, ts_time, ts_data, gd_row, gd_col)
    gui_ds{gd_row,gd_col} = ts_time;
    gui_ds{gd_row,gd_col+1} = ts_data;
end
