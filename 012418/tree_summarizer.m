...
while (not_exit)
   ...
   if isnan(distance_matrix(next_idx))
    ...
    % you probably got here if you're at the last possible merge
    if distance_matrix(min_idx) == distance_matrix(next_idx)
        not_exit = 0;
        break;
    end
   ...
   if isnan(distance_matrix(min_idx))
       ...
       % Add children nodes
       temp_data = ts_1(distance_matrix_loc(min_idx):distance_matrix_loc(min_idx)+MAGIC_mp_seg_len-1);
       %temp_merge = [distance_matrix_loc(min_idx),distance_matrix_loc(min_idx)+MAGIC_mp_seg_len-1];
       temp_merge = distance_matrix_loc(min_idx);
       child_1 = binarytree(temp_data, temp_merge, NaN, NaN,level);
       
       temp_data = ts_1(distance_matrix_loc(next_idx):distance_matrix_loc(next_idx)+MAGIC_mp_seg_len-1);
       temp_merge = distance_matrix_loc(next_idx);
       %temp_merge = [distance_matrix_loc(next_idx),distance_matrix_loc(next_idx)+MAGIC_mp_seg_len-1];
       child_2 = binarytree(temp_data, temp_merge, NaN, NaN, level);

       % Add merged node as root of tree
        tree = binarytree(child_1.ts_seg, child_1.index, child_1, child_2, level+1);
